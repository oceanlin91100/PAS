...
{
    public class WFPORGMRepository
    {
        private string ConnectionString = string.Empty;
        public WFPORGMRepository()
        {
        ...
        public DataTable GetWFPORGM(string employeeno)
        {
            ...
            using (var connection = new OleDbConnection(ConnectionString))
            {
                ...
                var sqlQuery = "SELECT * FROM WFDLIB.WFPORGM WHERE EMPNO ='" + employeeno + "'";
        ...
        public DataTable GetVP(string employeeno)
        {
            ...
            {
                connection.Open(); if (connection.State != ConnectionState.Open) { return dt; }
                var sqlQuery = "SELECT EMPNO, EMPNAME, EMPACCT, BRHCOD, BRHNAMB, DEPTCD, DEPTNAM, TEAMCD, MANGER, EMPNAME01, MNGACCT, ORGPWD, ORGNAME, 1 as level, EMPNO AS sort_path FROM WFDLIB.WFPORGM WHERE EMPNO ='" + employeeno + "'";
                var command = new OleDbCommand(sqlQuery, connection);
                OleDbDataAdapter da = new OleDbDataAdapter(command);
                da.Fill(dt);
            }
        ...
        public DataTable GetUnits(string unitCode)
        {
            ...
            {
                connection.Open(); if (connection.State != ConnectionState.Open) { return dt; }
                var sqlQuery = "SELECT BRHCOD,DEPTCD, DEPTNAM, EMPNO FROM WFDLIB.WFPORGM WHERE SUBSTR(BRHCOD, 1, 3) = '585' AND DEPTCD IN ('" + unitCode + "')  AND MANGER = '86760' GROUP BY BRHCOD, DEPTCD, DEPTNAM, EMPNO";
                var command = new OleDbCommand(sqlQuery, connection);
                OleDbDataAdapter da = new OleDbDataAdapter(command);
                da.Fill(dt);
            }
        ...
        public DataTable GetUnits()
        {
            ...
            {
                connection.Open(); if (connection.State != ConnectionState.Open) { return dt; }
                var sqlQuery = "SELECT BRHCOD,DEPTCD, DEPTNAM, EMPNO FROM WFDLIB.WFPORGM WHERE MANGER = '86760' AND EMPNO <>'86760' GROUP BY BRHCOD, DEPTCD, DEPTNAM, EMPNO";                
                var command = new OleDbCommand(sqlQuery, connection);
                OleDbDataAdapter da = new OleDbDataAdapter(command);
                da.Fill(dt);
            }
        ...
        public DataTable GetFlow(string employeeNo)
        {
            ...
            using (var connection = new OleDbConnection(ConnectionString))
            {
                ...
                " FROM WFDLIB.WFPORGM as child ,n WHERE n.EMPNO = child.MANGER) " +
                "SELECT * FROM n ";
                var command = new OleDbCommand(sqlQuery, connection);
                OleDbDataAdapter da = new OleDbDataAdapter(command);
                da.Fill(dt);
